...
export const contactSection = defineType({
  ...
  fields: [...headingFields,
    ...
    defineField({ name: "form", title: "نصوص نموذج الطلب", type: "object", validation: (r) => r.required(), fields: [
      ...[["nameLabel","عنوان حقل الاسم"],["emailLabel","عنوان حقل البريد الإلكتروني"],["phoneLabel","عنوان حقل الهاتف"],["companyLabel","عنوان حقل الشركة"],["serviceLabel","عنوان حقل الخدمة"],["servicePlaceholder","النص الافتراضي لاختيار الخدمة"],["messageLabel","عنوان حقل الرسالة"],["submitText","نص زر الإرسال"],["sendingText","النص أثناء الإرسال"],["successHeading","عنوان رسالة النجاح"],["successMessage","نص رسالة النجاح"],["resetText","نص إرسال طلب آخر"],["errorFallback","رسالة الخطأ العامة"]].map(([name, title]) => defineField({ name, title, type: name.includes("Message") || name === "errorFallback" ? "text" : "string", validation: (r) => r.required().max(name === "successMessage" || name === "errorFallback" ? 260 : 100).custom(nonEmpty) })),
      ...[["namePlaceholder","المثال داخل حقل الاسم"],["emailPlaceholder","المثال داخل حقل البريد الإلكتروني"],["phonePlaceholder","المثال داخل حقل الهاتف"],["companyPlaceholder","المثال داخل حقل الشركة"],["messagePlaceholder","المثال داخل حقل الرسالة"]].map(([name, title]) => defineField({ name, title, type: "string", validation: (r) => r.max(120) })),
    ] }),
