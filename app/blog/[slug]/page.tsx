      ...
      <article>
        <header className="bg-[oklch(0.982_0.008_178)] pb-14 pt-10 sm:pb-20 sm:pt-14">
          <div className="container-custom">
            ...
            <div className="mx-auto max-w-[58rem] text-center">
              {article.categories?.length ? <div className="mb-5 flex flex-wrap justify-center gap-2">{article.categories.map((category) => <span key={category._id} className="rounded-full bg-accent px-3 py-1 text-sm font-bold text-accent-foreground">{category.title}</span>)}</div> : null}
              <h1 className="text-[clamp(2.4rem,5.5vw,4.8rem)] font-black leading-[1.12] tracking-[-0.04em] text-secondary">{article.title}</h1>
              <p className="mx-auto mt-6 max-w-[45rem] text-lg leading-8 text-[oklch(0.42_0.035_210)] sm:text-xl sm:leading-9">{article.excerpt}</p>
              <div className="mt-7 flex flex-wrap items-center justify-center gap-x-5 gap-y-2 text-sm text-muted-foreground">
                <span className="inline-flex items-center gap-2"><UserRound className="size-4" aria-hidden="true" />{article.author.name}</span>
        ...
        <div className="container-custom py-14 sm:py-20">
          <div className="mx-auto max-w-[52rem]">
            <section aria-labelledby="direct-answer-heading" className="mb-12 rounded-[1.5rem] border border-[oklch(0.84_0.035_180)] bg-[oklch(0.96_0.025_176)] p-6 sm:p-8">
              <h2 id="direct-answer-heading" className="text-lg font-black text-secondary">الخلاصة</h2>
              <p className="mt-3 text-lg leading-8 text-[oklch(0.34_0.035_220)]">{article.directAnswer}</p>
            </section>
